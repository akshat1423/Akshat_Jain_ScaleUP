-- Schema Fix Safe - Fix RLS policies without conflicts
-- This version checks for existing policies before creating new ones

-- Fix community_roles policies
DO $$ 
BEGIN
...
-- Fix communities policies (check if they exist first)
DO $$ 
BEGIN
    -- Only drop and recreate if they exist
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'communities' AND policyname = 'Anyone can view public communities') THEN
        DROP POLICY "Anyone can view public communities" ON communities;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'communities' AND policyname = 'Community creators can update their communities') THEN
        DROP POLICY "Community creators can update their communities" ON communities;
    END IF;
...
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'community_members' AND policyname = 'Users can view community members') THEN
        DROP POLICY "Users can view community members" ON community_members;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'community_members' AND policyname = 'Users can join communities') THEN
        DROP POLICY "Users can join communities" ON community_members;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'community_members' AND policyname = 'Users can leave communities') THEN
        DROP POLICY "Users can leave communities" ON community_members;
    END IF;
...
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'notifications' AND policyname = 'Users can view notifications') THEN
        DROP POLICY "Users can view notifications" ON notifications;
    END IF;
